# examples/instance_interaction_example.py
import inspect
import json
from dataclasses import dataclass, field, make_dataclass
from typing import Any, Callable, List, Dict, Tuple, Type, Optional

# Assuming the llm_tap package is installed or src is in PYTHONPATH
# If running directly from repo root, Python might not find llm_tap without specific PYTHONPATH manipulation.
# For simplicity in this example, we might need to adjust imports if llm_tap is not 'installed'.
# One common way for local dev: add parent dir to sys.path
import sys
import os
sys.path.insert(0, os.path.abspath(os.path.join(os.path.dirname(__file__), '..')))

from llm_tap import InstanceRegistry, LLMInstanceInterface
from llm_tap.llm import to_json_schema, as_tool # For verification if needed
...
class Thermostat:
    ...
    def get_current_temperature(self) -> float:
        ...
        print(f"[Thermostat {self.location}] Getting current temperature: {self.current_temperature}°{self.unit}")
    ...
    def set_temperature(self, temperature: float, unit: Optional[str] = None):
        ...
        if unit:
            self.unit = unit
        print(f"[Thermostat {self.location}] Setting target temperature from {old_temp}° to {self.target_temperature}°{self.unit}")
...
class LightSwitch:
    ...
    def get_status(self) -> Dict[str, Any]:
        ...
        status = {"name": self.name, "is_on": self.is_on_state, "brightness": self.brightness if self.is_on_state else 0}
...
def main():
    ...
    registry.register_instance(
        ...
        allowlist={"methods": ["get_current_temperature", "set_temperature", "change_mode"]}
    )
    ...
    registry.register_instance(
        ...
        allowlist={"methods": ["turn_on", "turn_off"]} # get_status is NOT allowlisted
    )
    ...
    simulated_llm_call_set_temp = {
        ...
        "arguments": json.dumps({"temperature": 23.5, "unit": "Celsius"}) # Arguments as JSON string
    }

    try:
        instance_id, method_name, args = interface.parse_llm_action(simulated_llm_call_set_temp)
        print(f"Parsed Action: Instance='{instance_id}', Method='{method_name}', Args={args}")
    ...
    try:
        instance_id, method_name, args = interface.parse_llm_action(simulated_llm_call_light_on)
        print(f"Parsed Action: Instance='{instance_id}', Method='{method_name}', Args={args}")
    ...
    try:
        instance_id, method_name, args = interface.parse_llm_action(simulated_llm_call_bedroom_status)
        print(f"Parsed Action: Instance='{instance_id}', Method='{method_name}', Args={args}")
    ...
    try:
        instance_id, method_name, args = interface.parse_llm_action(simulated_llm_call_fake_method)
        print(f"Parsed Action: Instance='{instance_id}', Method='{method_name}', Args={args}")
    ...
    try:
        instance_id, method_name, args = interface.parse_llm_action(simulated_llm_call_invalid_param)
        print(f"Parsed Action: Instance='{instance_id}', Method='{method_name}', Args={args}")
