...
class MockThermostat:
    ...
    def set_temperature(self, temperature: float, unit: Optional[str] = "Celsius") -> str:
...
class TestInstanceRegistry(unittest.TestCase):
    ...
    def test_register_instance_success(self):
        self.registry.register_instance(
            "thermo_living", self.thermostat_instance, "Living room thermostat",
            {"methods": ["set_temperature", "get_temperature"]}
        )
        self.assertIn("thermo_living", self.registry.instances)
        self.assertEqual(self.registry.get_instance("thermo_living"), self.thermostat_instance)
        self.assertEqual(self.registry.get_instance_description("thermo_living"), "Living room thermostat")

    def test_register_instance_duplicate_id(self):
        self.registry.register_instance("thermo1", self.thermostat_instance, "Desc1", {"methods": ["get_temperature"]})
        with self.assertRaisesRegex(ValueError, "Instance ID 'thermo1' is already registered."):
            self.registry.register_instance("thermo1", self.light_instance, "Desc2", {"methods": ["turn_on"]})

    def test_register_instance_method_not_found(self):
        with self.assertRaisesRegex(ValueError, "Method 'non_existent_method' not found or not callable on instance 'thermo1'."):
            self.registry.register_instance(
                "thermo1", self.thermostat_instance, "Desc1",
                {"methods": ["non_existent_method"]}
            )

    def test_register_instance_attribute_as_method(self):
        with self.assertRaisesRegex(ValueError, "Method 'id' not found or not callable on instance 'thermo1'."):
            self.registry.register_instance(
                "thermo1", self.thermostat_instance, "Desc1",
    ...
    def test_register_instance_internal_method_on_allowlist(self):
        ...
        self.registry.register_instance(
            "thermo_hidden", self.thermostat_instance, "Thermo with hidden method",
    ...
    def test_list_available_instances(self):
        self.registry.register_instance("t1", self.thermostat_instance, "Thermo 1", {"methods": ["get_temperature"]})
        self.registry.register_instance("l1", self.light_instance, "Light 1", {"methods": ["turn_on"]})
        expected_list = {"t1": "Thermo 1", "l1": "Light 1"}
        self.assertEqual(self.registry.list_available_instances(), expected_list)
...
class TestLLMInstanceInterface(unittest.TestCase):
    def setUp(self):
        ...
        self.registry.register_instance(
            "thermostat", self.thermostat_instance, "Main thermostat",
            {"methods": ["set_temperature", "get_temperature", "method_with_no_params", "method_with_custom_object_param"]}
        )
        self.registry.register_instance(
            "light", self.light_instance, "Main light",
    ...
    def test_get_method_parameters_dataclass(self):
        ...
        param_dc_no_params = self.interface._get_method_parameters_dataclass(method_no_params)
    ...
    def test_generate_tool_schemas_for_instance(self):
        schemas = self.interface.generate_tool_schemas_for_instance("thermostat")
        self.assertEqual(len(schemas), 4) # set_temperature, get_temperature, method_with_no_params, method_with_custom_object_param

        set_temp_schema = next(s for s in schemas if s["function"]["name"] == "thermostat.set_temperature")
        self.assertIn("Main thermostat", set_temp_schema["function"]["description"])
        self.assertIn("Sets the thermostat temperature.", set_temp_schema["function"]["description"])
        self.assertIn("temperature", set_temp_schema["function"]["parameters"]["properties"])
        self.assertIn("unit", set_temp_schema["function"]["parameters"]["properties"])
        self.assertEqual(set_temp_schema["function"]["parameters"]["properties"]["unit"]["default"], "Celsius")

        get_temp_schema = next(s for s in schemas if s["function"]["name"] == "thermostat.get_temperature")
        self.assertEqual(len(get_temp_schema["function"]["parameters"]["properties"]), 0) # No params
        self.assertEqual(get_temp_schema["function"]["parameters"]["required"], [])


        custom_param_schema = next(s for s in schemas if s["function"]["name"] == "thermostat.method_with_custom_object_param")
        self.assertIn("custom_obj", custom_param_schema["function"]["parameters"]["properties"])
        ...
        self.assertEqual(custom_param_schema["function"]["parameters"]["properties"]["custom_obj"], {"type": "object"})
    ...
    def test_generate_helper_text(self):
        ...
        self.assertIn("    - set_temperature(temperature: float, unit: Optional[str] = 'Celsius')", helper_text)
        self.assertIn("    - get_temperature()", helper_text) # Python 3.9+ might show -> float
    ...
    def test_parse_llm_action(self):
        llm_output = {"name": "thermostat.set_temperature", "arguments": '{"temperature": 25.5, "unit": "F"}'}
        instance_id, method_name, args = self.interface.parse_llm_action(llm_output)
    ...
    def test_parse_llm_action_no_dot(self):
        ...
        with self.assertRaisesRegex(ValueError, "LLM output name is not in 'instance_id.method_name' format."):
            self.interface.parse_llm_action(llm_output)

    def test_parse_llm_action_invalid_json(self):
        llm_output = {"name": "thermostat.set_temperature", "arguments": '{"temp": 25.5, unit: "F"}'} # Invalid JSON
        # The original error was json.JSONDecodeError
        # My implementation produces a ValueError that wraps the JSONDecodeError
        # I will use the error message produced by my implementation.
        with self.assertRaisesRegex(ValueError, "Failed to parse arguments JSON string:"):
            self.interface.parse_llm_action(llm_output)

    def test_execute_action_success(self):
        result = self.interface.execute_action("thermostat", "set_temperature", {"temperature": 22.0, "unit": "C"})
        self.assertEqual(result, "Temperature set to 22.0°C for main_thermo")
        self.assertEqual(self.thermostat_instance.temp, 22.0)

        result_no_params = self.interface.execute_action("thermostat", "method_with_no_params", {})
    ...
    def test_execute_action_not_allowlisted(self):
        ...
        with self.assertRaisesRegex(PermissionError, "Method '_internal_method' is not allowlisted for instance 'thermostat'."):
            self.interface.execute_action("thermostat", "_internal_method", {})

    def test_execute_action_method_does_not_exist_on_instance_but_allowlisted_by_mistake(self):
        ...
        self.registry.allowlists["thermostat"]["methods"].append("fake_method_on_allowlist") # Manually add to allowlist
        
        with self.assertRaises(AttributeError): # Because getattr will fail
             self.interface.execute_action("thermostat", "fake_method_on_allowlist", {})
    ...
    def test_execute_action_instance_not_found(self):
        with self.assertRaises(KeyError): # From registry.get_instance
            self.interface.execute_action("non_existent_thermostat", "set_temperature", {})

    def test_execute_action_method_raises_exception(self):
        with self.assertRaisesRegex(ValueError, "Value cannot be zero."):
            self.interface.execute_action("light", "method_that_fails", {"value": 0})
        
        # Check it works with non-zero
        result = self.interface.execute_action("light", "method_that_fails", {"value": 2})
