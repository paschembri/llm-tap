...
class LLMInstanceInterface:
    ...
    def _get_method_parameters_dataclass(self, method: Callable) -> Type[dataclasses.dataclass]:
        fields = []
        ...
        for name, param in sig.parameters.items():
            ...
            default_value = param.default if param.default != inspect.Parameter.empty else dataclasses.MISSING
    ...
    def generate_tool_schemas_for_instance(self, instance_id: str) -> List[Dict]:
        instance = self.registry.get_instance(instance_id)
        allowlist = self.registry.get_instance_allowlist(instance_id)
        instance_description = self.registry.get_instance_description(instance_id)
        tool_schemas = []
        ...
        for method_name in allowlist.get("methods", []):
            ...
            tool_spec = as_tool(json_schema) # This should return a dict based on typical OpenAI tool structure

            tool_spec["function"]["name"] = f"{instance_id}.{method_name}"
            
            method_docstring = inspect.getdoc(method) or "No detailed description available."
    ...
    def generate_all_tool_schemas(self) -> List[Dict]:
        ...
        for instance_id in self.registry.list_available_instances():
            all_schemas.extend(self.generate_tool_schemas_for_instance(instance_id))
        return all_schemas

    def generate_helper_text(self) -> str:
        helper_lines = ["You can interact with the following pre-existing objects:"]
        for instance_id, description in self.registry.list_available_instances().items():
            ...
            for method_name in allowlist.get("methods", []):
                ...
                for p_name, p_obj in sig.parameters.items():
                    ...
                    param_str = f"{p_name}: {p_obj.annotation if p_obj.annotation != inspect.Parameter.empty else 'Any'}"
                ...
                helper_lines.append(f"    - {method_name}({params_str}){doc_str}")
        return "\n".join(helper_lines)

    def parse_llm_action(self, llm_output: Dict) -> Tuple[str, str, Dict]:
        try:
            name_parts = llm_output["name"].split(".", 1)
            if len(name_parts) != 2:
                raise ValueError("LLM output name is not in 'instance_id.method_name' format.")
            ...
            raise ValueError(f"Failed to parse LLM action output: {llm_output}. Error: {e}")
        except json.JSONDecodeError as e:
            raise ValueError(f"Failed to parse arguments JSON string: {arguments_str}. Error: {e}")
            
        return instance_id, method_name, parsed_args

    def execute_action(self, instance_id: str, method_name: str, arguments: Dict) -> Any:
        instance = self.registry.get_instance(instance_id) # Raises KeyError if not found
        allowlist = self.registry.get_instance_allowlist(instance_id)
